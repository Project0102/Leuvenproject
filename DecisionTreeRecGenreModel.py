...
class DecisionTreeRecGenreModel:
    def __init__(self, source_matrix, genre_matrix, top_items, top_genres,
                 tree_depth_threshold=6, lambda_value=7,
                 top_items_num=1000, top_genres_num=50):
        ...
        # genre related data
        self.top_genres_num = top_genres_num
        self.top_genres_list = top_genres
        self.real_genres_num = genre_matrix.shape[1]
        genre_data = find(genre_matrix)
        genre_users_list = genre_data[0]
        genre_list = genre_data[1]
        self.rG = {}
        # self.track_genre_mapping = track_genre_dict
        ...
        # Generate rGenres, rG genre data
        print("Generate the genre interactions dictionary")
        self.rGenres = list(set(genre_list))
        # self.sum_genre_ratings = 0
        # self.sum_square_genre_ratings = 0
        for user_id, genre_id in zip(genre_users_list, genre_list):
            # if user_id in self.rU:
            rating = genre_matrix[user_id, genre_id]
            self.rG.setdefault(user_id, {})[genre_id] = rating
            # self.sum_genre_ratings += rating
            # self.sum_square_genre_ratings += rating ** 2

        '''        
        self.user_item_genre_rating = {}
        self.user_item_genre_average_rating = {}
        self.rU_item_average_rating = {}
        for user_id, items in self.rU.items():
            for item_id, item_rating in items.items():
                genre_ratings = []
                genre_id_list = self.track_genre_mapping.get(item_id, [])
                for genre_id in genre_id_list:
                    genre_rating = self.rG.get(user_id, {}).get(genre_id)
                    if genre_rating:
                        self.user_item_genre_rating.setdefault(user_id, {}).setdefault(item_id, {})[genre_id] = genre_rating
                        genre_ratings.append(genre_rating)
                if len(genre_ratings) > 0:
                    genre_average_rating = np.mean(genre_ratings)
                    self.user_item_genre_average_rating.setdefault(user_id, {})[item_id] = genre_average_rating
                    self.rU_item_average_rating.setdefault(user_id, {})[item_id] = (item_rating + genre_average_rating)/2
                else:
        ...
        self.sum_cur_node_t_genre = np.zeros(self.real_genres_num)
        self.sum_cur_square_node_t_genre = np.zeros(self.real_genres_num)
        self.cnt_cur_node_t_genre = np.zeros(self.real_genres_num)
    ...
    def populate_stats_node_t(self):
        ...
        for user_id in self.rG:
            user_all_items_rating_id_genre = np.array(list(self.rG[user_id].keys()))
            user_all_rating_genre = np.array(list(self.rG[user_id].values()))
            self.sum_cur_node_t_genre[user_all_items_rating_id_genre[:]] += user_all_rating_genre[
                                                                            :]  # - self.biasU[user_id]
            self.sum_cur_square_node_t_genre[user_all_items_rating_id_genre[:]] += (user_all_rating_genre[
                                                                                    :]) ** 2  # - self.biasU[user_id]
            self.cnt_cur_node_t_genre[user_all_items_rating_id_genre[:]] += 1
        # self.sum_cur_node_t += self.sum_genre_ratings
        # self.sum_cur_square_node_t += self.sum_square_genre_ratings
    ...
    def generate_decision_tree(self, users_bound_for_node, chosen_item_id):
        ...
        # Choose popular rated items
        # num_rec = np.zeros(self.real_genres_num)
        # for user_id in self.tree[users_bound_for_node[0]:(users_bound_for_node[1] + 1)]:
        #    if user_id in self.rG:
        #        user_all_genres_rating_id = np.array(list(self.rG[user_id].keys()))
        #        num_rec[user_all_genres_rating_id[:]] += 1
        # sub_genre_id = np.argsort(num_rec)[::-1][:self.top_genres_num]
        sub_item_id = self.rI  # np.array(self.top_items_list)
        sub_genre_id = self.rGenres  # np.array(self.top_genres_list)
        ...
        # print("num sub_genre_id: ", len(sub_genre_id))

        # print("Items loaded")
        # print("start error")
        # Find optimum item to split
        min_cnt_node_t, min_sum_node_t, min_sum_square_node_t, min_cnt_node_t_genre, \
            min_sum_node_t_genre, min_sum_square_node_t_genre, optimum_item_id, optimum_genre_id \
            = self.populate_items_error_list(chosen_item_id, sub_item_id, sub_genre_id, users_bound_for_node)
        ...
        for user_id in self.tree[users_bound_for_node[0]:(users_bound_for_node[1] + 1)]:
            genre_rating_dict_values = self.rG.get(user_id, {}).get(optimum_genre_id)
            genre_rating = genre_rating_dict_values if genre_rating_dict_values is not None else None
            # print("user_id",user_id)
            if genre_rating is None or optimum_item_id not in self.rU[user_id] or self.rU[user_id][optimum_item_id] < 0:
                list_unknown.append(user_id)
            elif self.rU[user_id][optimum_item_id] > 50 and genre_rating > 50:
        ...
        self.sum_cur_node_t_genre = min_sum_node_t_genre[:, 0]
        self.sum_cur_square_node_t_genre = min_sum_square_node_t_genre[:, 0]
        self.cnt_cur_node_t_genre = min_cnt_node_t_genre[:, 0]
        ...
        self.sum_cur_node_t_genre = min_sum_node_t_genre[:, 1]
        self.sum_cur_square_node_t_genre = min_sum_square_node_t_genre[:, 1]
        self.cnt_cur_node_t_genre = min_cnt_node_t_genre[:, 1]
        ...
        self.sum_cur_node_t_genre = min_sum_node_t_genre[:, 2]
        self.sum_cur_square_node_t_genre = min_sum_square_node_t_genre[:, 2]
        self.cnt_cur_node_t_genre = min_cnt_node_t_genre[:, 2]
    ...
    def populate_items_error_list(self, chosen_item_id, sub_item_id, sub_genre_id, users_bound_for_node):
        # items_error_list = {}
        ...
        min_sum_node_t_genre = np.zeros((self.real_genres_num, 3))
        min_sum_square_node_t_genre = np.zeros((self.real_genres_num, 3))
        min_cnt_node_t_genre = np.zeros((self.real_genres_num, 3))
        ...
        if len(node_users) > 0:
            for item_id in set(sub_item_id) - set(chosen_item_id):
                ...
                for genre_id in sub_genre_id:
                    ...
                    sum_node_t_genre = np.zeros((self.real_genres_num, 3))
                    sum_square_node_t_genre = np.zeros((self.real_genres_num, 3))
                    cnt_node_t_genre = np.zeros((self.real_genres_num, 3))
                    for user_rating in user_item_rating_node_t:
                        ...
                        genre_rating_dict_values = self.rG.get(user_rating[0], {}).get(genre_id)
                        genre_rating = genre_rating_dict_values if genre_rating_dict_values is not None else None

                        if genre_rating:
                            # print("User Rating: ", user_rating[1],"Genre Rating: ", genre_rating)
                            user_all_items_rating_id_genre = np.array(list(self.rG[user_rating[0]].keys()))
                            user_all_rating_genre = np.array(list(self.rG[user_rating[0]].values()))

                            valid_ratings_like = np.logical_and(user_rating[1] > 50, genre_rating > 50)
                            valid_ratings_dislike = np.logical_and(
                                np.logical_and(user_rating[1] >= 0, user_rating[1] <= 50),
                                np.logical_and(genre_rating >= 0, genre_rating <= 50))
                            ...
                            if valid_ratings_like:
                                ...
                                sum_node_t_genre[user_all_items_rating_id_genre[:], 0] += user_all_rating_genre[:]
                                sum_square_node_t_genre[user_all_items_rating_id_genre[:], 0] += user_all_rating_genre[
                                                                                                 :] ** 2
                                cnt_node_t_genre[user_all_items_rating_id_genre[:], 0] += 1
                            ...
                            # calculate sum_tL for node LIKE
                                ...
                                sum_node_t_genre[user_all_items_rating_id_genre[:], 1] += user_all_rating_genre[:]
                                sum_square_node_t_genre[user_all_items_rating_id_genre[:], 1] += user_all_rating_genre[
                                                                                                 :] ** 2
                                cnt_node_t_genre[user_all_items_rating_id_genre[:], 1] += 1
                    ...
                    sum_node_t_genre[:, 2] = self.sum_cur_node_t_genre[:] - sum_node_t_genre[:, 0] - sum_node_t_genre[:, 1]
                    sum_square_node_t_genre[:, 2] = self.sum_cur_square_node_t_genre[:] - sum_square_node_t_genre[:, 0] - sum_square_node_t_genre[:, 1]
                    cnt_node_t_genre[:, 2] = self.cnt_cur_node_t_genre[:] - cnt_node_t_genre[:, 0] - cnt_node_t_genre[:, 1]

                    error_total = self.calculate_item_error(sum_node_t, sum_square_node_t, cnt_node_t, sum_node_t_genre,
                                                            sum_square_node_t_genre, cnt_node_t_genre)
                    # print ("item_id: ", item_id, ", genre_id: ", genre_id, ", Error: ", error_total)
                    if error_total < min_error_item:
                        min_sum_node_t = sum_node_t
                        min_sum_square_node_t = sum_square_node_t
                        min_cnt_node_t = cnt_node_t
                        min_sum_node_t_genre = sum_node_t_genre
                        min_sum_square_node_t_genre = sum_square_node_t_genre
                        min_cnt_node_t_genre = cnt_node_t_genre
                        min_error_item = error_total
                        min_item_id = item_id
                        min_genre_id = genre_id

        # if not items_error_list:
        #    items_error_list[0] = 0
        return min_cnt_node_t, min_sum_node_t, min_sum_square_node_t, min_cnt_node_t_genre, min_sum_node_t_genre, min_sum_square_node_t_genre, min_item_id, min_genre_id

    def calculate_item_error(self, sum_node_t, sum_square_node_t, cnt_node_t, sum_node_t_genre, sum_square_node_t_genre,
                             cnt_node_t_genre):
        # Calculate error for one item-split in current node
        error_item = np.sum(sum_square_node_t - (sum_node_t ** 2) / (cnt_node_t + 1e-9))
        error_genre = np.sum(sum_square_node_t_genre - (sum_node_t_genre ** 2) / (cnt_node_t_genre + 1e-9))
        #consider equal weights for item error and genre error
        #avoid average for saving computation
        return error_item + error_genre
